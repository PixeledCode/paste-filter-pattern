...
} from "@twilio-paste/core/combobox";
import type { Item } from "@twilio-paste/combobox/dist/types";
import type { UseMultipleSelectionStateChange } from "@twilio-paste/core/dropdown-library";
...
export const HostNameFilter: React.FC<HostNameFilterProps> = ({
  ...
  const filteredItems = React.useMemo(
    () => getFilteredItems(inputValue, items as string[]),
    [inputValue, items]
  );
  ...
  const state = useMultiselectCombobox({
    initialSelectedItems:
      (selectedMoreFilters && (selectedMoreFilters.hostName as Item[])) || [],
  ...
  return (
    <Box>
      ...
      <Box marginTop="space70">
        <CheckboxGroup
          name={`recently-used-hostName-${useUID()}`}
          legend="Recently used"
        >
